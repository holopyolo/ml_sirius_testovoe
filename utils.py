...
from llama_index.core.text_splitter import SentenceSplitter, SentenceSplitter
...
def get_llm(llm_hf_name,
    ...
    llm = AutoModelForCausalLM.from_pretrained(llm_hf_name, **model_args).eval()
...
def hybrid_retriever(embedder, texts, retriever, question, llm, tokenizer, generation_config, k=1, k_final=5):
    qustionds_cands = create_queries(llm, tokenizer, generation_config, question)
    ranked_lists = []
    for candidat_q in qustionds_cands:
        if '?' not in candidat_q:
            continue
        ranked_lists.append(retriever(embedder, texts, candidat_q, k, get_indices=True))
...
def retrieve_context_topk(embedder, texts, question, k=1, get_indices=False):
    ...
    scores = np.dot(embeddings, question_embedding) / (np.linalg.norm(embeddings, axis=1) * np.linalg.norm(question_embedding))
...
def pipeline_rag_answer(llm, 
    ...
    relevant_content = ''.join(retriever_pipeline(embedder=embedder_model, 
                                                  texts=prepare_context(context, pipeline), 
